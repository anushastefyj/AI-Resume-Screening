...
job_desc = st.sidebar.text_area("Paste Job Description", height=200, placeholder="Job Title: Python Developer\nRequired Skills: Python, AWS, Docker, Django...")
...
if st.sidebar.button("Analyze Resume"):
    if not resume:
        ...
        with st.spinner("Analyzing resume..."):
            try:
                response = requests.post(f"{API_URL}/match-resume", files={"file": (resume.name, resume.getvalue(), "application/pdf")}, data={"job_description": job_desc}, timeout=30)
